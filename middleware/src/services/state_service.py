...
def handle_accepted_state(request_id, external_requestId, agent_url):
    ...
    if not restart_agent_with_tenant_config(agent_url, request_data):
...
def update_request_state_handler(external_requestId, state, agent_url):
    ...
    try:
        ...
        if state.lower() != 'accepted':
            ...
            success, error = handle_accepted_state(request_id, external_requestId, agent_url)
